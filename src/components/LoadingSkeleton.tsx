...
export default function LoadingSkeleton({ variant = 'default' }: LoadingSkeletonProps) {
    ...
    return (
      <div className="animate-pulse space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 h-48 bg-slate-800/40 rounded-2xl"></div>
        </div>
        
        {/* Quick Actions */}
        <div>
          <div className="h-6 w-32 bg-slate-800/40 rounded mb-4"></div>
  ...
  if (variant === 'profile') {
    return (
      <div className="animate-pulse space-y-6">
        {/* Header */}
        <div className="h-12 w-48 bg-slate-800/40 rounded-lg"></div>
        
        {/* Form Fields */}
        <div className="bg-slate-800/40 rounded-2xl p-8 space-y-6">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i}>
          ...
          <div className="h-12 bg-gradient-to-r from-violet-500/20 to-indigo-500/20 rounded-lg"></div>
        </div>
      </div>
    )
  }
  ...
  if (variant === 'resume') {
    return (
      <div className="animate-pulse space-y-8">
        {/* Header */}
        <div className="h-12 w-64 bg-slate-800/40 rounded-lg"></div>
        ...
        <div className="bg-slate-800/40 rounded-2xl p-8 space-y-6">
          <div className="text-center">
            <div className="h-16 w-32 bg-slate-700/50 rounded-lg mx-auto mb-2"></div>
            <div className="h-4 w-40 bg-slate-700/50 rounded mx-auto"></div>
          </div>
